...
namespace XLY.SF.Project.Domains
{
    ...
    #region 分区参数基本信息  fs_fun_15接口使用

    [StructLayout(LayoutKind.Sequential, Pack = 1)]
    public struct VOL_BPB_INFO
    {
        public Byte fs_type;               //文件系统类型
        public UInt32 bytes_per_sec;       //每扇区字节数
        public UInt32 secs_per_clust;           //每簇扇区数
        public UInt32 bytes_per_clust;          //每簇块字节数
        public UInt32 data_aera_offset_sec;     //数据区开始偏移扇区
        public UInt64 part_offset_sec;            //分区偏移扇区数
        public UInt64 part_total_secs;            //分区总扇区数
        public UInt32 first_clust;              //开始簇号
        public UInt64 total_clust;            //分区总簇数
        public UInt32 clust_bmp_len;
        public UInt64 root_ino;               //根节点号 20160815
        public Int32 check_sum;				//校验值
    }
