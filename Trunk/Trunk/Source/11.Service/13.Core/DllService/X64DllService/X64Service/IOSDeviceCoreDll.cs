...
namespace X64Service
{
    public static class IOSDeviceCoreDll
    {
        ...
        {
            DevicePreDeal(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"Lib\vcdllX64\IphoneDevices"));
        }
        ...
        {
            var res = _StartDeviceMonitoring(startMonitoringCallback);
        ...
        private static readonly object DoStartDeviceMonitoringLocker = new object();
        ...
        private static void DoStartDeviceMonitoring()
        {
            ...
            {
                lock (DoStartDeviceMonitoringLocker)
                {
                    if (!IsStartDeviceMonitoring)
                    {
                        var res = _StartDeviceMonitoring((i, s) => 0);
        ...
        /// <param name="puid">设备序列号</param>
        /// <returns></returns>
        [DllImport(IPhoneDeviceDll, EntryPoint = "closeIphoneFileService")]
        public static extern uint CloseIphoneFileService(string puid);
