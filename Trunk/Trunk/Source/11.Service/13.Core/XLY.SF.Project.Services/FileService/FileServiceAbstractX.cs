...
namespace XLY.SF.Project.Services
{
    ...
    public abstract class FileServiceAbstractX
    {
        ...
        private void GetFileSystemByDirScanCallBack(ref ScanCallBackData pdi)
        {
        ...
        public void ExportFileX(FNodeX file, string savePath, bool persistRelativePath = true, bool isMedia = false, bool isCover = false)
        {
            try
            {
                if (file.IsFolder)
                {
                    ExportFolder(file, savePath, persistRelativePath);
                }
                else
                {
        ...
        /// 导出文件夹及递归其下所有文件、文件夹
        /// </summary>
        private void ExportFolder(FNodeX file, string savePath, bool persistRelativePath = true)
        {
            var path = string.Empty;
            if (persistRelativePath)
            {
                path = FileHelper.ConnectPath(savePath, file.Directory, file.IsDelete ? "Del_" + file.FileName : file.FileName);
            }
            else
            {
                path = FileHelper.ConnectPath(savePath, file.IsDelete ? "Del_" + file.FileName : file.FileName);
            }
            //1.1创建文件夹
