...
namespace XLY.SF.Project.Devices
{
    public sealed class AndroidHelper
    {
        ...
        public string CopyFile(Device device, string sourcePath, string targetPath, IAsyncTaskProgress asyn, bool persistRelativePath = true)
        {
            ...
            {
                //直接copy单文件的
                return CopySingleFile(device, sourcePath, targetPath, asyn, persistRelativePath);
            }
            //folder
            ...
            CopyFolder(device, files, targetPath, asyn, persistRelativePath);
            return path;
        }
        ...
        private void CopyFolder(Device device, List<LSFile> files, string targetPath, IAsyncTaskProgress asyn, bool persistRelativePath = true)
        {
            ...
            foreach (var file in files)
            {
                if (file.IsFolder)
                {
                    ...
                    {
                        CopyFolder(device, fs, targetPath, asyn, true);
                    }
                    else
                    {
                        CopyFolder(device, fs, Path.Combine(targetPath, file.Name), asyn, false);
                    }
                }
                else if (file.Size > 0)
                {
                    CopySingleFile(device, file, targetPath, asyn, persistRelativePath);
                }
            }
        }
        ...
        public string CopySingleFile(Device device, LSFile file, string targetPath, IAsyncTaskProgress asyn, bool persistRelativePath = true)
        {
            ...
            var res = CopySingleFile(device, file.FullPath, targetPath, asyn, persistRelativePath);
            return res;
        }
        ...
        public List<LSFile> FindRootFiles(Device device)
        {
            try
            {
                LSFileReceiver receiver = new LSFileReceiver();
                receiver.Source = "";
                ExecuteRemoteAutoCommand("ls -l", device, receiver);
                return receiver.Files;
            }
            catch (Exception ex)
            {
                LoggerManagerSingle.Instance.Error(ex, "扫描文件发生异常");
                return null;
            }
        }
