...
namespace XLY.SF.Project.Devices
{
    ...
    /// USB设备监听服务
    /// </summary>
    public sealed class UsbMonitorService : IDeviceMonitor
    {
        ...
        private void DealCallbackInThread(Tuple<int, IntPtr> arg)
        {
            ...
            else   //设备断开
            {
                ...
                if (infoStr.Length > 2)
                {
                    ...
                    DeviceDisconnected?.Invoke(new UsbDevice() { VID = devInfo.VID, PID = devInfo.PID });
