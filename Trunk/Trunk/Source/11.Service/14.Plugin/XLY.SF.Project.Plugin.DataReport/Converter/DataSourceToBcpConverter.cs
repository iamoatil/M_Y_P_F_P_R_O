...
/* ==============================================================================
* Assembly   ：	XLY.SF.Project.Plugin.DataReport.DataSourceToBcpConverter
* Description：	  
* Author     ：	fhjun
...
namespace XLY.SF.Project.Plugin.DataReport
{
    ...
    public class DataSourceToBcpConverter
    {
        ...
        /// </summary>
        private static WeakReference<List<BcpTable>> _wLstBcpTableConfigure = new WeakReference<List<BcpTable>>(null);
        private static List<BcpTable> LstBcpTableConfigure
        {
            get
            {
                if (!_wLstBcpTableConfigure.TryGetTarget(out List<BcpTable> ss))
                {
                    ss = Serializer.DeSerializeFromXML<List<BcpTable>>(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Config\\bcp.cfg.xml"));
                    _wLstBcpTableConfigure.SetTarget(ss);
                }
                return ss;
            }
        }
        ...
        /// 转换函数
        /// </summary>
        private static WeakReference<Dictionary<string, MethodInfo>> _wconverterFuns = new WeakReference<Dictionary<string, MethodInfo>>(null);
        private static Dictionary<string, MethodInfo> _converterFuns
        { 
            get
            {
                if (!_wconverterFuns.TryGetTarget(out Dictionary<string, MethodInfo> ss))
                {
                    ss = new Dictionary<string, MethodInfo>();
                    _wconverterFuns.SetTarget(ss);
                }
                return ss;
            }
        }
        ...
        private const string INDEX_FILE_NAME = "GAB_ZIP_INDEX.xml";
        #endregion
        ...
        private static object ConverterResult(string funName, object[] funParams, object currentValue)
        {
            ...
            {
                return _converterFuns[funName].Invoke(BcpConverter.Instance, new object[] { currentValue, funParams });
            }
            else
            {
                MethodInfo mi = typeof(BcpConverter).GetMethod(funName);
                if (mi != null)
                {
