...
namespace XLY.SF.Project.EarlyWarningView
{
    class EarlyWarningSqlDb:IDisposable
    {
        ...
        {
            _path = Path.GetFullPath(@"EarlyWarning\智能提取\data.db");         
        }
        ...
        internal void WriteResult(IEnumerable<dynamic> result, string tableName,Type type)
        {
            ...
            //记录，当前数据库中没有条目
            List<AbstractDataItem> ls = new List<AbstractDataItem>();
            foreach (AbstractDataItem item in result)
            {
                SQLiteCommand cmd = new SQLiteCommand(string.Format("select * from {0} where MD5 = '{1}' ", tableName, item.MD5), _dbConnection);
            ...
            foreach (AbstractDataItem item in ls)
            {
                StringBuilder sb = new StringBuilder();
