...
namespace XLY.SF.Project.PreviewFiles.Language
{
    public class LanguageHelperSingle
    {
        ...
        public void SwitchLanguage(LanguageType langType = LanguageType.Cn, bool isLoadPageLanguage = false)
        {
            ...
            switch (langType)
            {
                case LanguageType.En:
                    PageLanguageHelper.LoadPageLanguage(new Uri("Pack://application:,,,/XLY.SF.Project.PreviewFiles;Component/Language/Language_En.xml", UriKind.RelativeOrAbsolute));
                    _xmlDoc.LoadXml(Resource1.Language_En); break;
                case LanguageType.Cn:
                    PageLanguageHelper.LoadPageLanguage(new Uri("Pack://application:,,,/XLY.SF.Project.PreviewFiles;Component/Language/Language_Cn.xml", UriKind.RelativeOrAbsolute));
        ...
        public string GetEntityDescByKey(string propertyKey)
        {
            ...
            {
                return _xmlDoc.SelectSingleNode(propertyKey)?.InnerText ?? propertyKey;
            }
            return string.Empty;
        }
            ...
            lock (_flyWeightLanguages)
            {
                if (!string.IsNullOrWhiteSpace(languageKey))
                {
                    if (!_flyWeightLanguages.ContainsKey(languageKey))
                    {
                        ...
                        {
                            Framework.Log4NetService.LoggerManagerSingle.Instance.Error(ex, string.Format("获取{0}语言失败", languageKey));
                            return string.Empty;
                        }
                    }
                }
