...
namespace XLY.SF.Framework.Core.Base.MessageAggregation
{
    public class MsgAggregation
    {
        ...
        public void SendGeneralMsg(GeneralArgs msgData)
        {
            try
            {
                GalaSoft.MvvmLight.Messaging.Messenger.Default.Send<GeneralArgs>(msgData, msgData.MsgToken);
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="args">消息</param>
        public void SendSysMsg<TParam>(SysCommonMsgArgs<TParam> args)
        {
            GalaSoft.MvvmLight.Threading.DispatcherHelper.CheckBeginInvokeOnUI(() =>
            {
                GalaSoft.MvvmLight.Messaging.Messenger.Default.Send<SysCommonMsgArgs<TParam>>(args, args.MsgToken);
            });
        }
