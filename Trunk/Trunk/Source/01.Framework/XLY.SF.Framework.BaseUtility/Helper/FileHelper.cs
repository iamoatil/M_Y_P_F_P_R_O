...
namespace XLY.SF.Project.BaseUtility.Helper
{
    public class FileHelper
    {
        ...
        /// <summary>
        /// 获取新的名称，如果存在则在后面加(1)；
        /// newNameFormat：新文件的格式，比如 newfile{0}.txt
        /// </summary>
        /// <param name="existNames">当前已经存在的名称列表</param>
        /// <param name="newNameFormat"></param>
        /// <returns></returns>
        public static string GetUniqueName(IEnumerable<string> existNames, string newNameFormat)
        {
            int index = 1;
            string name = string.Format(newNameFormat, "");
            while (true)
            {
                if (existNames.FirstOrDefault(n => n == name) == null)
                {
                    break;
                }
                name = string.Format(newNameFormat, $"({index})");
                index++;
            }
            return name;
        }
